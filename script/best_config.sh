...
if [ x"$m" = x"vgg-n1-8180-1s" ]; then
  ...
  sleep 1
  # vgg19_conv4_2, 6.4T
  ./script/run.sh -c -i512 -h28 -o512 -H28 -v0 -n1 --blk-i=8 --blk-o=4 --blk-t=26 --execution-mode=0xa000 --streaming-input=1
  sleep 1
  # vgg19_conv5_1, 5.2T
  ./script/run.sh -c -i512 -h14 -o512 -H14 -v0 -n1 --blk-i=8 --blk-o=4 --blk-t=25 --execution-mode=0xa000
fi

if [ x"$m" = x"vgg-n1-8180-2s" ]; then
  export NSOCKETS=2
  sleep 1
  # vgg19_conv1_2, 12.8T
...
if [ x"$m" = x"resnet-n1-8180-1s" ]; then
  export NSOCKETS=1
  sleep 1
...
if [ x"$m" = x"resnet-n1-8180-2s" ]; then
  export NSOCKETS=2
  sleep 1
  # resnet50_res2a_branch2b, 8.4T
  ./script/run.sh -c -i64 -h56 -o64 -H56 -v0 -n1 --execution-mode=0xa061 --blk-i=4 --blk-o=2 --blk-t=13 --pat-o=2
  sleep 1
...
if [ x"$m" = x"resnet-n64-8180-2s" ]; then
  export NSOCKETS=2
  sleep 1
