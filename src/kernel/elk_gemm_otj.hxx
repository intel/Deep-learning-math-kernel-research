...
struct gemm_kernel_otj<GarrayTypes, V, Vx, ISA_SKX_AVX512,
  ...
  static inline typename std::enable_if<P == 1, void>::type op_gemm_OVT(
      ...
      __m<V> mmout[JO][T], __m<V> mmwei[P][JO], int V_, int _I2)
  {
#pragma nounroll
    for (int _V = 0; _V < V_ / P; ++_V) {
      ...
      unroll_for (_T, T) {
        __m<V> mmbcst = op_load_input<P>(xc, input, _V, 0, _T);
        unroll_for (_O, JO)
...
#pragma nounroll
    for (int _V = 0; _V < V_ / P; ++_V) {
      ...
      unroll_for (_T, T) {
        __m<V> mmbcst = op_load_input<P>(xc, input, _V, 1, _T);
        unroll_for (_O, JO)
...
#pragma nounroll
    for (int _V = 0; _V < V_ / P; ++_V) {
      ...
      unroll_for (_O, JO)
        mmwei[0][_O] = op_load_weights<JO, P>(xc, weights, _I2, _V + 1, 0, _O);
      unroll_for (_T, T) {
        __m<V> mmbcst = op_load_input<P>(xc, input, _V, 2, _T);
        unroll_for (_O, JO)
      ...
      unroll_for (_T, T) {
        __m<V> mmbcst = op_load_input<P>(xc, input, _V, 3, _T);
        unroll_for (_O, JO)
      ...
      ScaleType *weights_scale, ScaleType *weights_factor, int _O1, int _O0)
  {
    ...
    for (int _I2 = 0; _I2 < xc.I2 - 1; ++_I2) {
      op_gemm_OVT<JO, P>(
          xc, &md2(ainput, _I2, 0), weights, mmout, mmwei, V, _I2);
    }
    op_gemm_OVT<JO, P>(xc, &md2(ainput, xc.I2 - 1, 0), weights, mmout, mmwei,
        xc.Ir, xc.I2 - 1);
  ...
  template <int JO, int P>
  static inline typename std::enable_if<!std::is_same<InputType, uint8_t>::value
          && !has_Ir && (P == 1 || P == 2 || P == 4), void>::type
  op_gemm(elx_conv_params_t &xc, OutputType *output, InputType *input,
      WeightsType *weights, BiasType *bias, int attr, ScaleType *src_scale,
      ...
      ScaleType *weights_factor, int _O1, int _O0)
  {
    ...
    for (int _I2 = 0; _I2 < xc.I2; ++_I2) {
      op_gemm_OVT<JO, P>(xc, &md2(ainput, _I2, 0), weights, mmout, mmwei, V, _I2);
    }
    // store output
    unroll_for (_O, JO) {
      unroll_for (_T, T)
  ...
  template <int JO, int P>
  static inline typename std::enable_if<P == 1, void>::type op_int8_gemm_OVT(
      elx_conv_params_t &xc, InputType *input, WeightsType *weights,
      __i<V> mmout[JO][T], __i<V> mmwei[P][JO], int V_, int _I2)
  {
#pragma nounroll
    for (int _V = 0; _V < V_ / P; ++_V) {
      unroll_for (_O, JO)
      ...
      unroll_for (_T, T) {
        __i<V> bcast = op_int8_load_input<P>(input, _V, 0, _T);
        unroll_for (_O, JO)
  ...
  template <int JO, int P>
  static inline typename std::enable_if<P == 2, void>::type op_int8_gemm_OVT(
      elx_conv_params_t &xc, InputType *input, WeightsType *weights,
      __i<V> mmout[JO][T], __i<V> mmwei[P][JO], int V_, int _I2)
  {
#pragma nounroll
    for (int _V = 0; _V < V_ / P; ++_V) {
      ...
      unroll_for (_O, JO)
        mmwei[0][_O]
            = op_int8_load_weights<JO, P>(xc, weights, _I2, _V + 1, 0, _O);
      unroll_for (_T, T) {
        __i<V> bcast = op_int8_load_input<P>(input, _V, 1, _T);
        unroll_for (_O, JO)
  ...
  template <int JO, int P>
  static inline typename std::enable_if<P == 4, void>::type op_int8_gemm_OVT(
      elx_conv_params_t &xc, InputType *input, WeightsType *weights,
      __i<V> mmout[JO][T], __i<V> mmwei[P][JO], int V_, int _I2)
  {
#pragma nounroll
    for (int _V = 0; _V < V_ / P; ++_V) {
      ...
      unroll_for (_O, JO)
        mmwei[0][_O]
            = op_int8_load_weights<JO, P>(xc, weights, _I2, _V + 1, 0, _O);
      unroll_for (_T, T) {
        __i<V> bcast = op_int8_load_input<P>(input, _V, 2, _T);
        unroll_for (_O, JO)
      ...
      unroll_for (_O, JO)
        mmwei[1][_O]
            = op_int8_load_weights<JO, P>(xc, weights, _I2, _V + 1, 1, _O);
      unroll_for (_T, T) {
        __i<V> bcast = op_int8_load_input<P>(input, _V, 3, _T);
        unroll_for (_O, JO) {
      ...
      ScaleType *weights_factor, int _O1, int _O0)
  {
    ...
    for (int _I2 = 0; _I2 < xc.I2 - 1; ++_I2) {
      op_int8_gemm_OVT<JO, P>(
          xc, &md2(ainput, _I2, 0), weights, mmout, mmwei, V, _I2);
    }
    op_int8_gemm_OVT<JO, P>(xc, &md2(ainput, xc.I2 - 1, 0), weights, mmout,
        mmwei, xc.Ir, xc.I2 - 1);
  ...
  template <int JO, int P>
  static inline typename std::enable_if<!has_Ir && (P == 1 || P == 2 || P == 4),
      void>::type
  op_gemm(elx_conv_params_t &xc, OutputType *output, uint8_t *input,
      int8_t *weights, BiasType *bias, int attr, ScaleType *src_scale,
      ScaleType *src_factor, ScaleType *weights_scale,
  ...
  static inline typename std::enable_if<P == 1, void>::type op_conv_OVT(
      elx_conv_params_t &xc, InputType *input, WeightsType *weights,
...
#pragma nounroll
    ...
    for (int _kh = khs; _kh < khe; ++_kh) {
      for (int _I2 = 0; _I2 < I2; ++_I2) {
        ...
        for (int _kw = kws; _kw < kwe; ++_kw) {
          op_conv_OVT<JO, P, 0, T>(xc, &md2(ainput, _I2, 0),
              &md3(aweights, _kh, _kw, 0), mmout, mmwei, V, _kh, _kw, _I2);
        }
        // left
        if (_wt == 0) {
          int _kw = 0; // K = 3, 5, 7
          op_conv_OVT<JO, P, AKW, T>(xc, &md2(ainput, _I2, 0),
              &md3(aweights, _kh, _kw, 0), mmout, mmwei, V, _kh, _kw, _I2);
          if (K > 3) {
            _kw = 1; // K = 5, 7
            op_conv_OVT<JO, P, AKW-1, T>(xc, &md2(ainput, _I2, 0),
                &md3(aweights, _kh, _kw, 0), mmout, mmwei, V, _kh, _kw, _I2);
          }
          if (K > 5) {
            _kw = 2; // K = 7
            op_conv_OVT<JO, P, AKW-2, T>(xc, &md2(ainput, _I2, 0),
                &md3(aweights, _kh, _kw, 0), mmout, mmwei, V, _kh, _kw, _I2);
          }
        }
        // right
        if (_wt == xc.wt - 1) {
          int _kw = K - 1; // K = 3, 5, 7
          op_conv_OVT<JO, P, 0, T-AKW>(xc, &md2(ainput, _I2, 0),
              &md3(aweights, _kh, _kw, 0), mmout, mmwei, V, _kh, _kw, _I2);
          if (K > 3) {
            _kw = K - 2; // K = 5, 7
            op_conv_OVT<JO, P, 0, T-(AKW-1)>(xc, &md2(ainput, _I2, 0),
                &md3(aweights, _kh, _kw, 0), mmout, mmwei, V, _kh, _kw, _I2);
          }
          if (K > 5) {
            _kw = K - 3; // K = 7
            op_conv_OVT<JO, P, 0, T-(AKW-2)>(xc, &md2(ainput, _I2, 0),
                &md3(aweights, _kh, _kw, 0), mmout, mmwei, V, _kh, _kw, _I2);
          }
        }
      } // I2 loop
  ...
  template <int JO, int P, int FROM, int TO>
  static inline typename std::enable_if<P == 2, void>::type op_conv_OVT(
      elx_conv_params_t &xc, InputType *input, WeightsType *weights,
...
#pragma nounroll
  ...
  template <int JO, int P, int FROM, int TO>
  static inline typename std::enable_if<P == 4, void>::type op_conv_OVT(
      elx_conv_params_t &xc, InputType *input, WeightsType *weights,
      __m<V> mmout[JO][T], __m<V> mmwei[P][JO], int V_, int _kh, int _kw,
      int _I2)
  {
      ...
      int khs, int khe, int kws, int kwe, int attr)
  {
    ...
    for (int _kh = khs; _kh < khe; ++_kh) {
      ...
      for (int _kw = kws; _kw < kwe; ++_kw) {
        ...
        // Ir
        int _I2 = xc.I2 - 1;
        op_conv_OVT<JO, 1, 0, T>(xc, &md2(ainput, _I2, 0),
      ...
      if (_wt == 0) {
        {
          ...
          // Ir
          int _I2 = xc.I2 - 1;
          op_conv_OVT<JO, 1, AKW, T>(xc, &md2(ainput, _I2, 0),
              &md3(aweights, _kh, _kw, 0), mmout, mmwei[0], Ir, _kh, _kw, _I2);
        }
        if (K > 3) {
          int _kw = 1; // K = 5, 7
          ...
          // Ir
          int _I2 = xc.I2 - 1;
          op_conv_OVT<JO, 1, AKW - 1, T>(xc, &md2(ainput, _I2, 0),
              &md3(aweights, _kh, _kw, 0), mmout, mmwei[0], Ir, _kh, _kw, _I2);
        }
        if (K > 5) {
          int _kw = 2; // K = 7
          ...
          // Ir
          int _I2 = xc.I2 - 1;
          op_conv_OVT<JO, 1, AKW - 2, T>(xc, &md2(ainput, _I2, 0),
              &md3(aweights, _kh, _kw, 0), mmout, mmwei[0], Ir, _kh, _kw, _I2);
        }
      } // left
      ...
      if (_wt == xc.wt - 1) {
        {
          ...
          // Ir
          int _I2 = xc.I2 - 1;
          op_conv_OVT<JO, 1, 0, T - AKW>(xc, &md2(ainput, _I2, 0),
              &md3(aweights, _kh, _kw, 0), mmout, mmwei[0], Ir, _kh, _kw, _I2);
        }
        ...
        if (K > 3) {
          ...
          // Ir
          int _I2 = xc.I2 - 1;
          op_conv_OVT<JO, 1, 0, T - (AKW - 1)>(xc, &md2(ainput, _I2, 0),
              &md3(aweights, _kh, _kw, 0), mmout, mmwei[0], Ir, _kh, _kw, _I2);
        }
        if (K > 5) {
          int _kw = K - 3; // K = 7
          ...
          // Ir
          int _I2 = xc.I2 - 1;
          op_conv_OVT<JO, 1, 0, T - (AKW - 2)>(xc, &md2(ainput, _I2, 0),
              &md3(aweights, _kh, _kw, 0), mmout, mmwei[0], Ir, _kh, _kw, _I2);
        }
      } // right
    } // _kh loop
