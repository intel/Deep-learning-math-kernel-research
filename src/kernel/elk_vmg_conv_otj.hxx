...
struct vmg_conv_kernel_otj<GarrayTypes, V, Vx, ISA_SKX_AVX512,
      ...
      WeightsType *weights, const int _I2, const int _V, const int _P, const int _O)
  {
    ...
    if (F_traits<F>::is_compact_ir_weights) {
      ...
      MD5(WeightsType, aweights5, weights, xc.I2, C / P, P, O, V);
      if (std::is_same<WeightsType, float>::value) {
      ...
      MD6(WeightsType, aweights6, weights, JO, xc.ic34, xc.I2, C / P, P, V);
      if (std::is_same<WeightsType, float>::value) {
      ...
      int khs, int khe, int kws, int kwe, int attr)
  {
    ...
    MD3(WeightsType, aweights, weights, xc.kh, xc.kw, xc.O1 * I2 * C * O * V); // compact
    ...
    auto gemm_OVT = [&](InputType *input_, WeightsType *weights_,
      ...
      unroll_for(_V, C) {
        ...
        unroll_for(_T, T) {
          __m<V> mmbcst = op_load_input(xc, input_, _kh - AKH, _kw - AKW, _I2, _V, _T);
      ...
      int khs, int khe, int kws, int kwe, int attr)
  {
    ...
    MD3(WeightsType, aweights, weights, xc.kh, xc.kw, xc.O1 * xc.I2 * C * O * V); // compact
      ...
      for (int _kw = kws; _kw < kwe; ++_kw) {
        ...
        for (int _I2 = 0; _I2 < I2; ++_I2) {
          unroll_for(_V, C / P) {
            unroll_auto(_P, P) {
              unroll_for(_T, T) {
                __m<V> mmbcst = op_load_input(xc, input, _kh - AKH, _kw - AKW, _I2, _V * P + _P, _T);
...
#pragma nounroll
  ...
  {
    int Vr = F_traits<F>::is_compact_ir_weights ? xc.Ir : C;
