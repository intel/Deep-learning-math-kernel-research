...
#pragma omp parallel for collapse(1)
  for_each (_O2, xc.O2) {
    ...
    elk_gemm_ker<Type>((Type *)atoutput[_O2], (Type *)atinput[xc.I2 - 1],
        (Type *)atweights[_O2][xc.I2 - 1], T_, xc.Ir, V,
        zero_out && xc.I2 == 1);
  }
}
  ...
  {                                                                          \
    ...
    for (int _O2 = 0; _O2 < xc.O2; ++_O2) {                                  \
      ...
      for (int _V = 0; _V < xc.Ir; ++_V) {                                   \
        __m512 x;                                                            \
        __m512 w = _mm512_load_ps(atweights[_O2][xc.I2 - 1][_V]);            \
        float *x_ptr = &atinput[xc.I2 - 1][0][_V];                           \
        BOOST_PP_REPEAT(n, AVX512_FMA, nil);                                 \
      }                                                                      \
      BOOST_PP_REPEAT(n, AVX512_STORE, nil);                                 \
    }                                                                        \
