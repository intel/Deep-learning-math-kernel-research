...
#define K1(O2_, T_, P_)                                                        \
  ...
  void convolution_direct_1x1_kernel<Type, 1, O2_, T_, SSS, V, I, with_bias,   \
  ...
  template void                                                                \
  convolution_direct_1x1_kernel<float, 1, O2_, T_, SSS, 16, ISA_SKX_AVX512,    \
...
#define _MM_T_LOAD_WEIGHTS_P1(O_, P_)                                          \
  zmm_wei(O_, P_) = _mm512_load_ps(&md5(aweights5, O_, 0, xc.I2 - 1 , _V, 0))
#define MM_T_LOAD_WEIGHTS_P1(O_) MATRIX_OP(_MM_T_LOAD_WEIGHTS_P1, O_, 1);

#define K_GEMM_TAIL_FMA_P1(O_, T_, Os_)                                        \
  {                                                                            \
    __m512 bcast, zero;                                                        \
    MATRIX_OP(DEF_OUTPUT, O_, T_);                                             \
    MATRIX_OP(DEF_WEIGHTS, O_, 1);                                             \
    ...
    MD3(float, ainput3, &md2(ainput, xc.I2 - 1, 0), xc.t2, T_, V);             \
...
#define K_GEMM_TAIL_FMA_P2(O_, T_, Os_)                                        \
  {                                                                            \
    __m512 bcast, zero;                                                        \
    MATRIX_OP(DEF_OUTPUT, O_, T_);                                             \
    MATRIX_OP(DEF_WEIGHTS, O_, 2);                                             \
...
#define K_GEMM_TAIL_FMA_P4(O_, T_, Os_)                                        \
  {                                                                            \
    __m512 bcast, zero;                                                        \
    MATRIX_OP(DEF_OUTPUT, O_, T_);                                             \
    MATRIX_OP(DEF_WEIGHTS, O_, 4);                                             \
...
#define Kr2(O2_, T_, o0_, o1_, P0_, P1_)                                       \
  ...
  {                                                                            \
    ...
    MD2(Type, abias, bias, O2_, V);                                            \
                                                                               \
    K_GEMM_TAIL_FMA_P##P0_(o0_, T_, 0);                                        \
    K_GEMM_TAIL_FMA_P##P1_(o1_, T_, o0_);                                      \
  }                                                                            \
  template void                                                                \
  convolution_direct_1x1_kernel<float, 1, O2_, T_, SSS, 16, ISA_SKX_AVX512,    \
      BIAS(true), RELU(false), SUM(false)>::gemm_tail(elx_conv_t<float> &,     \
...
#define Kr3(O2_, T_, o0_, o1_, o2_, P0_, P1_, P2_)                             \
  ...
  {                                                                            \
    ...
    K_GEMM_TAIL_FMA_P##P0_(o0_, T_, 0);                                        \
    K_GEMM_TAIL_FMA_P##P1_(o1_, T_, o0_);                                      \
    K_GEMM_TAIL_FMA_P##P2_(o2_, T_, o0_ + o1_);                                \
  }                                                                            \
  template void                                                                \
  convolution_direct_1x1_kernel<float, 1, O2_, T_, SSS, 16, ISA_SKX_AVX512,    \
      BIAS(true), RELU(false), SUM(false)>::gemm_tail(elx_conv_t<float> &,     \
