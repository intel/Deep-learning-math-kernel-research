...
__TRANS_WEIGHTS(float, 6, 3, 16, ISA_GENERIC)
{
  const float z2 = 2.0f;
  const float z4 = 4.0f;
  const float z6 = 6.0f;
...
// float atweights[A][A][V][V] <- float aweights[K][K][V][V])
__TRANS_WEIGHTS(float, 6, 3, 16, ISA_SKX_AVX512)
{
  ENABLE_AVX512F();
...
TRANS_WEIGHTS(float, 6, 3, 16, ISA_GENERIC);
TRANS_WEIGHTS(float, 6, 3, 16, ISA_SKX_AVX512);
