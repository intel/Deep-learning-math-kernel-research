...
#undef K_GEMM_FMA_P2
#undef K_GEMM_FMA_P4
#undef K_GEMM_TAIL_FMA_P1
#undef K_GEMM_TAIL_FMA_P2
#undef K_GEMM_TAIL_FMA_P4
#undef K1
#undef K2
#undef K3
#undef Kr1
#undef Kr2
#undef Kr3
#undef MM_LOAD_OUTPUT
#undef _MM_LOAD_OUTPUT
#undef MM_COMPUTE_OUTPUT_P1
#undef _MM_COMPUTE_OUTPUT_P1
#undef __MM_COMPUTE_OUTPUT_P1
#undef MM_STORE_OUTPUT
#undef _MM_STORE_OUTPUT
#undef __MM_STORE_OUTPUT
#undef MM_COMPUTE_OUTPUT_P
#undef _MM_COMPUTE_OUTPUT_P
#undef __MM_COMPUTE_OUTPUT_P
#undef _MM_LOAD_WEIGHTS_P1
#undef _MM_T_LOAD_WEIGHTS_P1
#undef _MM_PRELOAD_WEIGHTS_P2
#undef _MM_LOAD_WEIGHTS_P2_1
#undef _MM_LOAD_WEIGHTS_P2_0
#undef _MM_PRELOAD_WEIGHTS_P4
#undef _MM_LOAD_WEIGHTS_P4_2
#undef _MM_LOAD_WEIGHTS_P4_3
#undef _MM_LOAD_WEIGHTS_P4_0
#undef _MM_LOAD_WEIGHTS_P4_1
#undef __MM_LOAD_OUTPUT
...
// iter_each (O_, O)
//   iter_each (T_, T)
//     zmm_out(O_, _T) = _mm512_load_ps(&md3(aoutput2, O_, T_, 0));
...
#define __MM_LOAD_OUTPUT(z, T_, O_)                                            \
  zmm_out(O_, T_) = _mm512_load_ps(&md3(aoutput2, O_, T_, 0));
...
#define K_GEMM_FMA_P1(O2_, O_, T_, Os_)                                        \
  {                                                                            \
    ...
    MD3(float, aoutput2, &md3(aoutput, Os_, 0, 0), O_, T_, V);                 \
...
#define _MM_COMPUTE_OUTPUT_P(z, T_, OP_)                                       \
  bcast                                                                        \
      = _mm512_broadcastss_ps(*(__m128 *)&md4(ainput, _I2, T_, _V, _1(OP_)));  \
...
#define K1(O2_, T_, P_)                                                        \
  ...
  template void                                                                \
  convolution_direct_1x1_kernel<float, 1, O2_, T_, CCC, 16, ISA_SKX_AVX512,    \
...
#define _MM_T_LOAD_WEIGHTS_P1(O_, P_)                                          \
  zmm_wei(O_, P_) = _mm512_load_ps(&md4(aweights4, xc.I2 - 1, _V, O_, 0))
#define MM_T_LOAD_WEIGHTS_P1(O_) MATRIX_OP(_MM_T_LOAD_WEIGHTS_P1, O_, 1);

#define MM_T_COMPUTE_OUTPUT_P1(O_, T_)                                         \
  BOOST_PP_REPEAT(T_, _MM_T_COMPUTE_OUTPUT_P1, O_);
#define _MM_T_COMPUTE_OUTPUT_P1(z, T_, O_)                                     \
  bcast = _mm512_broadcastss_ps(*(__m128 *)&md3(ainput, xc.I2 - 1, T_, _V));   \
  BOOST_PP_REPEAT(O_, __MM_T_COMPUTE_OUTPUT_P1, T_);
#define __MM_T_COMPUTE_OUTPUT_P1(z, O_, T_)                                    \
  zmm_out(O_, T_) = _mm512_fmadd_ps(zmm_wei(O_, 0), bcast, zmm_out(O_, T_));

#define K_GEMM_TAIL_FMA_P1(O2_, O_, T_, Os_)                                   \
  {                                                                            \
    __m512 bcast;                                                              \
    MATRIX_OP(DEF_OUTPUT, O_, T_);                                             \
    MATRIX_OP(DEF_WEIGHTS, O_, 1);                                             \
    ...
    pragma_unroll iter_each (_V, xc.Ir) {                                      \
      MM_T_LOAD_WEIGHTS_P1(O_);                                                \
      MM_T_COMPUTE_OUTPUT_P1(O_, T_);                                          \
    }                                                                          \
    MM_STORE_OUTPUT(O_, T_);                                                   \
  }

#define K_GEMM_TAIL_FMA_P2(O2_, O_, T_, Os_)                                   \
  {                                                                            \
    __m512 bcast;                                                              \
    MATRIX_OP(DEF_OUTPUT, O_, T_);                                             \
    MATRIX_OP(DEF_WEIGHTS, O_, 2);                                             \
...
#define K_GEMM_TAIL_FMA_P4(O2_, O_, T_, Os_)                                   \
  {                                                                            \
    __m512 bcast;                                                              \
    MATRIX_OP(DEF_OUTPUT, O_, T_);                                             \
    MATRIX_OP(DEF_WEIGHTS, O_, 4);                                             \
...
#define Kr1(O2_, T_, P_)                                                       \
  template <typename Type, const int V, const int I, const bool with_bias,     \
      const bool with_relu, const bool with_sum>                               \
  void convolution_direct_1x1_kernel<Type, 1, O2_, T_, CCC, V, I, with_bias,   \
      with_relu, with_sum>::gemm_tail(elx_conv_t<Type> &xc, Type *output,      \
      Type *input, Type *weights, Type *bias, bool reset_out)                  \
  {                                                                            \
    ENABLE_AVX512F();                                                          \
                                                                               \
  ...
  template void                                                                \
  convolution_direct_1x1_kernel<float, 1, O2_, T_, CCC, 16, ISA_SKX_AVX512,    \
      BIAS(true), RELU(false), SUM(false)>::gemm_tail(elx_conv_t<float> &,     \
      float *, float *, float *, float *, bool);                               \
  template void                                                                \
  convolution_direct_1x1_kernel<float, 1, O2_, T_, CCC, 16, ISA_SKX_AVX512,    \
      BIAS(false), RELU(false), SUM(false)>::gemm_tail(elx_conv_t<float> &,    \
      float *, float *, float *, float *, bool);                               \
  template void                                                                \
  convolution_direct_1x1_kernel<float, 1, O2_, T_, CCC, 16, ISA_SKX_AVX512,    \
      BIAS(true), RELU(true), SUM(false)>::gemm_tail(elx_conv_t<float> &,      \
      float *, float *, float *, float *, bool);                               \
  template void                                                                \
  convolution_direct_1x1_kernel<float, 1, O2_, T_, CCC, 16, ISA_SKX_AVX512,    \
      BIAS(false), RELU(true), SUM(false)>::gemm_tail(elx_conv_t<float> &,     \
      float *, float *, float *, float *, bool);

#define Kr2(O2_, T_, o0_, o1_, P0_, P1_)                                       \
  template <typename Type, const int V, const int I, const bool with_bias,     \
      const bool with_relu, const bool with_sum>                               \
  void convolution_direct_1x1_kernel<Type, 1, O2_, T_, CCC, V, I, with_bias,   \
      with_relu, with_sum>::gemm_tail(elx_conv_t<Type> &xc, Type *output,      \
...
#define Kr3(O2_, T_, o0_, o1_, o2_, P0_, P1_, P2_)                             \
  template <typename Type, const int V, const int I, const bool with_bias,     \
      const bool with_relu, const bool with_sum>                               \
  void convolution_direct_1x1_kernel<Type, 1, O2_, T_, CCC, V, I, with_bias,   \
      with_relu, with_sum>::gemm_tail(elx_conv_t<Type> &xc, Type *output,      \
