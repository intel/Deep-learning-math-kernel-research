...
#define K1(O2_, T_, P_)                                                        \
  ...
  void convolution_direct_1x1_kernel<Type, O2_, T_, V, I, OR(false), TR(true), \
      JAM(true), with_bias, with_relu, with_sum>::gemm(elx_conv_t<Type> &xc,   \
  ...
  template void convolution_direct_1x1_kernel<float, O2_, T_, 16,              \
      ISA_SKX_AVX512, OR(false), TR(true), JAM(true), BIAS(true), RELU(false), \
      SUM(false)>::gemm(elx_conv_t<float> &, float *, float *, float *,        \
      float *, bool);                                                          \
  template void convolution_direct_1x1_kernel<float, O2_, T_, 16,              \
      ISA_SKX_AVX512, OR(false), TR(true), JAM(true), BIAS(false),             \
      RELU(false), SUM(false)>::gemm(elx_conv_t<float> &, float *, float *,    \
      float *, float *, bool);
...
//    T=1..3:  bcast: 1, kernel: 8, outupt: 4..12 (pipeline: 4)
K1(4, 1, 4)
K1(4, 2, 4)
K1(4, 3, 4)
K1(4, 4, 2)
