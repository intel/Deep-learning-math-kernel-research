  ...
  iter_each (_I2, this->I2) {
    ...
    if (I == ISA_SKX_AVX512 && std::is_same<Type, float>::value) {
      if (stream_wei_) {
        ...
        iter_each (_hA, A) {
        iter_each (_iV, V) {
          _mm<V>::stream_ps(&md10(atweights, _oc4, _ic4, _oc3, _ic3, _wA, _hA,
                               _O2, _I2, _iV, 0),
              *((__m<V> *)&aout[_wA][_hA][_iV][0]));
        }}}
      } else {
        iter_each (_wA, A) {
        iter_each (_hA, A) {
        iter_each (_iV, V) {
          _mm<V>::store_ps(&md10(atweights, _oc4, _ic4, _oc3, _ic3, _wA, _hA,
                              _O2, _I2, _iV, 0),
              *((__m<V> *)&aout[_wA][_hA][_iV][0]));
        }}}
      }
  ...
  iter_each (_I2, this->I2) {
    ...
    if (I == ISA_SKX_AVX512 && std::is_same<Type, float>::value) {
      if (stream_wei_) {
        ...
        iter_each (_hA, A) {
        iter_each (_iV, V) {
          _mm<V>::stream_ps(&md10(atweights, _oc4, _ic4, _wA, _hA, _oc3, _ic3,
                               _O2, _I2, _iV, 0),
              *((__m<V> *)&aout[_wA][_hA][_iV][0]));
        }}}
      } else {
        ...
        iter_each (_hA, A) {
        iter_each (_iV, V) {
          _mm<V>::store_ps(&md10(atweights, _oc4, _ic4, _wA, _hA, _oc3, _ic3,
                              _O2, _I2, _iV, 0),
              *((__m<V> *)&aout[_wA][_hA][_iV][0]));
        }}}
      }
...
#pragma omp atomic read
  ...
  iter_each (_ic4, this->ic4) {
    iter_each (_oc, this->oc3 * this->O2/this->ic4) {
      iter_each (_T, Tz) {
        iter_each (_wA, A) {
        iter_each (_hA, A) {
#pragma omp simd
  ...
  if (inline_reduce) {
    ...
    iter_each (_T, Tz) {
      int _n, _oh, _ow, _hOA_end, _wOA_end;
      t2spato(_t2, _T, _n, _oh, _ow, _hOA_end, _wOA_end);
      iter_each (_hA, _hOA_end + 1) {
      iter_each (_wA, _wOA_end + 1) {
