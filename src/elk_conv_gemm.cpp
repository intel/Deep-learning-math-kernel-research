...
#define STORE(z, n, xxx) _mm512_store_ps(&atoutput(_O2, n, 0), t##n);

#define DEF_V_elk_gemm(z, n, xxx)                                             \
  template <>                                                                 \
  void elk_gemm<float, n, 16, ISA_SKX_AVX512>(                                \
    ...
    for (int _O2 = 0; _O2 < xc.O2; ++_O2) {                                   \
      BOOST_PP_REPEAT(n, AVX512_DEF, xxx)                                     \
                                                                              \
      if (zero_out) {                                                         \
        BOOST_PP_REPEAT(n, AVX512_ZERO, xxx)                                  \
      } else {                                                                \
        BOOST_PP_REPEAT(n, AVX512_LOAD, xxx)                                  \
      }                                                                       \
                                                                              \
      for (int _I2 = 0; _I2 < xc.I2; ++_I2) {                                 \
        for (int _V = 0; _V < 16; ++_V) {                                     \
          ...
          BOOST_PP_REPEAT(n, AVX512_FMA, xxx)                                 \
        }                                                                     \
      }                                                                       \
      BOOST_PP_REPEAT(n, STORE, xxx)                                          \
    }                                                                         \
  }
