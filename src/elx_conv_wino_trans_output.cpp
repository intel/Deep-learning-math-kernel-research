    ...
    int _O4, int _I4)
{
  ...
  auto writeout = [&](OutputType aout[A - K + 1][A - K + 1][V], int _O3,
    ...
    MD6(OutputType, aoutput1, &md2(aoutput0, _n, 0), xc->O4, xc->O3,
        xc->O2, V, xc->oh, xc->ow);

    for (int _hA = 0; _hA <= _hOA_end; ++_hA) {
      for (int _wA = 0; _wA <= _wOA_end; ++_wA) {
        if (is_Or) {
          if ((xc->with_ip_sum && !output_as_bfmt_) || _I4 > 0) {
            iter_each (_V, xc->Or)
    ...
    iter_each (_O2, xc->O2) {
      ...
      iter_each (_T, Tz) {
        ...
        ker_trans_output_(*xc, (OutputType *)aout, (float *)&In,
            (_I4 == xc->I4 - 1 || _I4 == -1) ? &md3(abias, _O3, _O2, 0)
                                                  : nullptr,
            0, -1);
...
{
  auto ker_trans_output = (xc->with_ip_sum || _I4 > 0)
      ? ker_trans_output_acc_
      : ker_trans_output_;
  auto ker_trans_output_tail = (xc->with_ip_sum || _I4 > 0)
      ? ker_trans_output0_acc_
      : ker_trans_output0_;

  // A, A, O3, O2, T, V -> n, oc2, oh, ow, V
  MD6(ToutputType, atoutput, toutput, A, A, xc->O3, xc->O2, Tz, V);
  MD7(OutputType, aoutput, output, xc->n, xc->O4, xc->O3, xc->O2,
      xc->oh, xc->ow, V);
  MD3(BiasType, abias, bias, xc->O3, xc->O2, V);
      ...
      iter_each (_T, Tz) {
        ...
        if (t2spato_o.is_border())
          ker_trans_output_tail(*xc, out, (float *)&In,
              (_I4 == -1 || _I4 == xc->I4 - 1) ? &md3(abias, _O3, _O2, 0)
                                                    : nullptr,
  ...
  iter_each (_O3, xc->O3) {
    iter_each (_O2, xc->O2) {
      ...
      iter_each (_T, Tz) {
        ...
        auto _oh = t2spato_o.t_;
        auto _ow = t2spato_o.l_;
        MD4(OutputType, aoutput1, &md4(aoutput0, _n, _oh, _ow, 0), xc->O4,
            xc->O3, xc->O2, V);
  ...
  THREAD_FOR(4, mthr_, ithr,
      [&](int _t2, int _O3, int _O2, int _T) {
        ...
        MD2(ToutputType, atoutput2, toutput, xc->t2,
            A * A * xc->T * xc->O3 * xc->O2 * V);
        MD3(BiasType, abias, bias, xc->O3, xc->O2, V);

        int Tz = _t2 == (xc->t2 - 1) ? xc->Tr : xc->T;
        MD6(ToutputType, atoutput, &md2(atoutput2, _t2, 0), A, A, xc->O3,
            xc->O2, Tz, V);
        alignas(64) union {
          __m<V> vin;
          TrOpType ain[V];
  ...
  THREAD_FOR(3, mthr_, ithr,
      [&](int _t2, int _O3, int _O2) {
        ...
        MD2(ToutputType, atoutput2, toutput, xc->t2,
            A * A * xc->T * xc->O3 * xc->O2 * V);
        int Tz = _t2 == (xc->t2 - 1) ? xc->Tr : xc->T;
        MD6(ToutputType, atoutput6, &md2(atoutput2, _t2, 0), A, A, xc->O3,
            xc->O2, Tz, V);
        alignas(64) OutputType aout[A - K + 1][A - K + 1][V];
        alignas(64) union {
          __m<V> vin;
