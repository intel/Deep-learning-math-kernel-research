...
void elx_conv_wino_trans_input_t<TinputType, InputType, OpType, I, A, K, V>
...
::__execute_post(TinputType *__restrict tinput,
  ...
  if (I == ISA_SKX_AVX512 && std::is_same<OpType, float>::value
  ...
  } else if (I == ISA_SKX_AVX512 && std::is_same<OpType, float>::value
...
void elx_conv_wino_trans_input_t<uint8_t, InputType, OpType, I, A, K, V>
::__execute_blocked(TscaleType *__restrict tinput_quant_scale,
    uint8_t *__restrict tinput_u8, OpType *__restrict tinput,
    ...
    if (_T < Tz) {
      ...
      if (xc->sampling_kind == CALIBRATED) {
        ...
        iter_each (_hA, A) {
          ...
          __m<V> a;
          if (std::is_same<OpType, float>::value)
            a = *(__m<V> *)&aout[_wA][_hA][0];
          else if (std::is_same<OpType, short>::value)
            a = _mm512_cvtepi32_ps(
                _mm512_cvtepi16_epi32(*(__m256i *)&aout[_wA][_hA][0]));
...
#pragma omp barrier
  thread_parallel_for<4>(mthr_, ithr, [&](int _t2, int _wA, int _hA, int _ic3) {
    ...
    MD8(InputType, ainput, input,
        xc->n, xc->ic4, xc->ic3, xc->I2, xc->Vx, xc->ih, xc->iw, V);
    int Tz = _t2 == (xc->t2 - 1) ? xc->Tr : xc->T;
    MD7(OpType, atinput7, &md2(atinput2, _t2, 0),
        xc->ic3, xc->I2, xc->Vx, Tz, A, A, V);
    MD7(uint8_t, atinput_u8, &md2(atinput2_u8, _t2, 0),
        A, A, xc->ic3, xc->I2, Tz, xc->Vx, V);
    iter_each (_T, Tz) {
      ...
      if (I == ISA_SKX_AVX512 && std::is_same<OpType, float>::value) {
        ...
        TscaleType *_mmax = (TscaleType *)&mmax;
        TscaleType *_mmin = (TscaleType *)&mmin;
        max = _mmax[0];
        min = _mmin[0];
    ...
    InputType *__restrict input, int _t2, int Tz)
{
  ...
  if (xc->sampling_kind == CALIBRATED) {
    ...
    if (std::is_same<OpType, float>::value) {
      MD7(OpType, atinput, tinput, xc->ic3, xc->I2, xc->Vx, Tz, A, A, V);
      ...
      iter_each (_Vx, xc->Vx) {
        ...
        iter_each (_T, Tz) {
          ...
          MD3(OpType, aout, &md7(atinput, _ic3, _I2, _Vx, _T, 0, 0, 0), A, A, V);
          using Array = OpType[A][A][V];
          InputType *in = &md8(ainput, t2spati_o.n_, 0, _ic3, _I2, _Vx, _ih, _iw, 0);
          if (!t2spati_o.is_border())
      ...
      TscaleType min = _mm<V>::reduce_min_ps(mmin);
      TscaleType max = _mm<V>::reduce_max_ps(mmax);

      TscaleType delta = max - min + 0.000001;
      TscaleType S = delta / INT8GEMM_TIN_MIN_MAX_QTSCALE;
      TscaleType repS = INT8GEMM_TIN_MIN_MAX_QTSCALE / delta;
      TscaleType z = std::ceil(-min * repS);
    ...
    if (std::is_same<OpType, float>::value) {
      MD5(OpType, atinput, tinput, xc->I2, xc->Vx, A, A, V);
      iter_each(_ic3, xc->ic3) {
        input_tile_iter<A, K> t2spati_o(_n, _t_off, xc->ht, xc->wt, xc->ih,
                                        xc->iw, xc->tp, xc->lp);
        iter_each (_T, Tz) {
          alignas(64) TscaleType mmax[A][A][V];
          alignas(64) TscaleType mmin[A][A][V];
          bool flush = true;
          iter_each (_I2, xc->I2) {
          iter_each (_Vx, xc->Vx) {
            auto _ih = t2spati_o.anchor_t_;
            ...
            MD3(OpType, aout, &md5(atinput, _I2, _Vx, 0, 0, 0), A, A, V);
            using Array = OpType[A][A][V];
            InputType *in = &md8(ainput, t2spati_o.n_, 0, _ic3, _I2, _Vx, _ih, _iw, 0);
            if (!t2spati_o.is_border())
              ker_trans_input_(*xc, *(Array *)&md3(aout, 0, 0, 0), in, 0, A - 1, 0, A - 1);
