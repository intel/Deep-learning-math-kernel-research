...
class elx_conv_direct_1x1_t : public elx_conv_t<Type> {
  ...
  template <int S, int F, int O, int T, bool Or, bool Tr, bool with_bias,
      bool with_relu, bool with_sum>
  using gemm_ker_cls_ = typename euler::gemm_kernel_otj<Type, V, I, S,
      estl::integer_sequence<F, O, T, false, Or, Tr, with_bias, with_relu,
          with_sum>>;
  using gemm_ker_fp_ = decltype(
      gemm_ker_cls_<0, 1, 1, 1, false, false, false, false, false>::execute);
